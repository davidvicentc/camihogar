...
export function AdminUsersManager({ initialUsers }: { initialUsers: UserRow[] }) {
  ...
  function remove(id: string) { if (!window.confirm("¿Eliminar este usuario?")) return; startTransition(async () => { const result = await deleteAdminUser(id); if (!result.ok) { setError(result.error ?? "No se pudo eliminar."); return; } setUsers((current) => current.filter((user) => user._id !== id)); }); }
