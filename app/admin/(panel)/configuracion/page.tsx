...
export default async function AdminConfiguracionPage() {
  ...
  return <div className="space-y-10"><header><h1 className="font-display text-3xl font-semibold text-brand-dark">Configuración</h1><p className="mt-1 text-sm text-brand-taupe">Gestiona los datos públicos, las vitrinas del home, redes y footer.</p></header><SiteSettingsForm initialSettings={settings} /><HomeProductOrderManager initialOrder={settings.homeProductOrder} bestsellers={products} featured={products.filter((product) => product.isFeatured)} /></div>;
}
